 ...
 * @author rnavarro
 */
public class TableListenerViñedo implements TableModelListener {

    private final Database db;

    public TableListenerViñedo(Database d) {
        super();
        db = d;
    }
        ...
        if (aux.isEmpty()) {
            ...
            try {
                if (validacionDatos(aux) > 0) {
                    JOptionPane.showMessageDialog(null, "El viñedo " + aux + " ya se encuentra en la base de datos.", "Mensaje", JOptionPane.INFORMATION_MESSAGE);

                } else {
                    Component rootPane = null;
                    int input = JOptionPane.showConfirmDialog(rootPane, "¿Desea guardar el cambio realizado en el ID = " + modelo.getValueAt(row, 0) + " en campo " + colName + " por " + aux.toUpperCase() + "?", "Modificar viñedo", 0);
                    if (input == 0) {
                        String sql = String.format(
                                "UPDATE winestore.ref_vineyards SET vineyard_name = \'%s\' WHERE vineyard_code = \'%s\'",
            ...
            /* String sql = String.format(
                "UPDATE winestore.ref_vineyards SET %s = \'%s\' WHERE vineyard_code = \'%s\'",
